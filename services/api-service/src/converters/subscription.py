    ...
    def parse(sub_key: str) -> ParsedSubscription | None:
        ...
        try:
            ...
            if "@" in symbol_part:
                ...
                data_type, params = SubscriptionKeyParser._parse_data_type(
                    data_type_part
                )
            else:
                symbol = symbol_part
...
class StreamParser:
    ...
    def parse(stream_name: str) -> StreamInfo:
        ...
        try:
            ...
            return StreamInfo(
                symbol=symbol, data_type=data_type, interval=interval, is_valid=True
            )

        except Exception as e:
            return StreamInfo(
                symbol="", data_type="", is_valid=False, error_message=f"解析异常: {e}"
            )
...
def stream_to_subscription_key(
    stream_name: str, exchange: str = "BINANCE"
) -> str | None:
    """便捷函数：流名称转订阅键"""

...
class TestSpotQuotesSubscription(SimpleE2ETestBase):
    """现货报价订阅测试"""
    ...
    async def test_quotes_subscription(self):
        ...
        print(f"现货报价: {quotes_count} 条 QUOTES 数据（格式验证通过，包含 ask/bid/spread）")
...
async def run_test():
    ...
    try:
        async with test:
            await test.setup()
            result = await test.test_quotes_subscription()
            return result

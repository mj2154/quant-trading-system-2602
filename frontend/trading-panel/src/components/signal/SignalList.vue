      ...
      >
        ...
        <div class="card-content">
          ...
          <div class="info-row time">
            ...
            <span class="value">{{ formatTime(signal.computedAt) }}</span>
          </div>
        </div>
      </n-card>
    </div>
    ...
    <n-modal v-model:show="showDetail" preset="card" title="信号详情" style="width: 600px">
      <n-descriptions v-if="selectedSignal" :column="2" label-placement="top" bordered>
        ...
        <n-descriptions-item label="信号值">
          <n-tag v-if="selectedSignal.signalValue !== null" :type="selectedSignal.signalValue ? 'success' : 'error'">
            {{ selectedSignal.signalValue ? '做多' : '做空' }}
          </n-tag>
          <span v-else>无信号</span>
        </n-descriptions-item>
        <n-descriptions-item label="触发类型">{{ selectedSignal.triggerType || '-' }}</n-descriptions-item>
        <n-descriptions-item label="计算时间" :span="2">{{ formatTime(selectedSignal.computedAt) }}</n-descriptions-item>
        <n-descriptions-item label="信号原因" :span="2">{{ selectedSignal.signalReason || '-' }}</n-descriptions-item>
        <n-descriptions-item v-if="selectedSignal.sourceSubscriptionKey" label="订阅键" :span="2">
          {{ selectedSignal.sourceSubscriptionKey }}
        </n-descriptions-item>
...
const totalPages = computed(() => {
  const total = store.alertSignalQueryParams.pageSize || 20

...
## 8.14 告警启用/禁用时的订阅管理设计

### 8.14.1 问题背景

> **本节说明**：8.14 是 8.5 节「订阅管理设计」的补充，专注于 signal-service 在告警启用/禁用时的订阅行为。

当告警配置被禁用时（`is_enabled = false`），信号服务不再需要接收该告警对应的K线数据更新。然而，当前实现存在以下问题：

**问题表现**：
1. 禁用告警后，信号服务仍然接收该K线周期的实时数据更新
2. `realtime_data.subscribers` 数组中仍然包含 `signal-service`
3. binance-service 的 `full_sync()` 会保持对该K线周期的WebSocket订阅
4. 造成资源浪费：服务处理不需要的数据，WebSocket连接持续保持

**根本原因**：
- 设计文档（01-task-subscription.md）定义了 `subscribers` 数组机制：当订阅源列表为空时，应删除行并通知币安服务取消订阅
- signal-service 的实现只在 `subscription_key` 改变时清理 subscribers
- 当 `is_enabled` 从 `true` 变为 `false` 时，没有触发 subscribers 清理

### 8.14.2 设计方案

**核心原则**：
- 告警禁用时：signal-service 应从 `realtime_data.subscribers` 中移除自己
- 告警启用时：signal-service 应将自己的 ID 添加到 `subscribers` 数组
- 订阅行为应与 `is_enabled` 状态保持一致

**订阅状态与告警启用状态的关系**：

| 告警状态 | signal-service 行为 | subscribers 数组 |
|---------|-------------------|-----------------|
| `is_enabled = true` | 添加订阅（如果不存在） | 包含 `signal-service` |
| `is_enabled = false` | 移除订阅 | 从数组移除 `signal-service` |
| 告警删除 | 移除订阅 | 从数组移除 `signal-service` |

**设计要点**：

1. **订阅决策基于 `is_enabled`，而非告警是否存在**
   - 只要 `is_enabled = true`，就应该保持订阅
   - 只要 `is_enabled = false`，就应该移除订阅

2. **数据库 subscribers 数组与内存状态分离**
   - 内存中的 `_alerts` 字典包含所有告警配置（包括禁用的）
   - 数据库的 `subscribers` 数组只包含当前需要订阅的（enabled 的）服务
   - 这样设计的好处：重启服务时能正确恢复订阅状态

3. **删除告警时也要清理订阅**
   - 如果某个 subscription_key 下的所有告警都被删除或禁用，应移除 subscribers

### 8.14.3 订阅管理流程

#### 8.14.3.1 告警启用时（is_enabled: false → true）

```
1. 前端更新 alert_configs.is_enabled = true
2. 数据库触发 alert_config.update 通知
3. signal-service 收到通知，调用 _reload_single_alert()
4. 检查 is_enabled 变化：
   - 旧值: false, 新值: true
   - 需要添加订阅
...
#### 8.14.3.2 告警禁用时（is_enabled: true → false）

```
1. 前端更新 alert_configs.is_enabled = false
2. 数据库触发 alert_config.update 通知
3. signal-service 收到通知，调用 _reload_single_alert()
4. 检查 is_enabled 变化：
   - 旧值: true, 新值: false
...
```
1. 前端删除告警配置
2. 数据库触发 alert_config.delete 通知
3. signal-service 收到通知，调用 _handle_alert_delete()
4. 从内存中移除告警
...
**启动时恢复**：
- signal-service 启动时，只为 `is_enabled = true` 的告警创建订阅
- 禁用告警后的重启：重启后不会重新订阅（因为告警本身就是禁用的）
...
两者协同工作：
- **is_enabled 变化时**：既更新内存字段（8.13.6），又更新数据库订阅（8.14）
- **subscription_key 变化时**：8.13.6 清理旧索引 + 8.14 清理旧订阅
- **删除告警时**：8.13.6 清理内存索引 + 8.14 清理数据库订阅

**与 8.5 节的关系**：

8.14 是 8.5 节「订阅管理设计」的补充：
- 8.5 定义了订阅的基础机制（`subscribers` 数组、触发器）
- 8.14 补充了 signal-service 在告警启用/禁用时的订阅行为

### 8.14.5 binance-service 设计说明
...
| 场景 | 设计决策 |
|------|----------|
| 禁用告警 | 从 `realtime_data.subscribers` 移除 `signal-service` |
| 启用告警 | 添加 `signal-service` 到 `realtime_data.subscribers` |
| 删除告警 | 从 `realtime_data.subscribers` 移除 `signal-service` |
| subscription_key 改变 | 清理旧的订阅，建立新的订阅 |
| binance-service full_sync | 按设计正常工作（subscribers 变空时行被删除） |
| 启用告警后恢复 | 从 `klines_history` 加载缓存，实时数据自动补充 |
...
1. **禁用告警**：
   - `realtime_data.subscribers` 中移除 `signal-service`
   - binance-service 取消对应 WebSocket 订阅

2. **启用告警**：
   - `realtime_data.subscribers` 中添加 `signal-service`
   - binance-service 恢复 WebSocket 订阅
   - K 线缓存正确初始化（从历史数据加载）

3. **删除告警**：
   - 如果没有其他告警使用该 subscription_key，则移除订阅

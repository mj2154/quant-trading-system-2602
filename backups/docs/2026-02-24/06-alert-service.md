...
CREATE TABLE IF NOT EXISTS alert_configs (
    ...
    condition_type VARCHAR(50) NOT NULL,  -- PRICE_ABOVE, PRICE_BELOW, RSI_ABOVE, etc.
...
- **RSI_ABOVE**: RSI 高于阈值
- **RSI_BELOW**: RSI 低于阈值
- **MA_CROSS_UP**: 移动平均线向上穿越
- **MA_CROSS_DOWN**: 移动平均线向下穿越
...
```sql
-- 告警新增通知（统一包装格式）
CREATE OR REPLACE FUNCTION notify_alert_config.new()
RETURNS TRIGGER AS $$
BEGIN
...
CREATE TRIGGER trigger_alert_config_new
    ...
    EXECUTE FUNCTION notify_alert_config.new();

-- 告警更新通知（统一包装格式）
CREATE OR REPLACE FUNCTION notify_alert_config.update()
RETURNS TRIGGER AS $$
BEGIN
...
CREATE TRIGGER trigger_alert_config_update
    ...
    FOR EACH ROW
    EXECUTE FUNCTION notify_alert_config.update();

-- 告警删除通知（统一包装格式）
CREATE OR REPLACE FUNCTION notify_alert_config.delete()
RETURNS TRIGGER AS $$
BEGIN
...
CREATE TRIGGER trigger_alert_config_delete
    ...
    FOR EACH ROW
    EXECUTE FUNCTION notify_alert_config.delete();
```

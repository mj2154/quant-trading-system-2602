...
```sql
-- 任务创建通知（统一包装格式）
CREATE OR REPLACE FUNCTION notify_task.new()
RETURNS TRIGGER AS $$
BEGIN
...
CREATE TRIGGER trigger_task_new
    ...
    FOR EACH ROW
    EXECUTE FUNCTION notify_task.new();
...
CREATE TRIGGER trigger_task_completed
    ...
    EXECUTE FUNCTION notify_task.completed();
...
```json
{
  ...
  "data": {
    ...
    "content": {
      "total_wallet_balance": "105.39513480",
      "total_margin_balance": "105.39513480",
